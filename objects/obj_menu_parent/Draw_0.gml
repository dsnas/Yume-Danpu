	...
	if (is_array(lvl[l].option) == true)
	{
		for (var o = 0; o < array_length(lvl[l].option); o++)
		{
			...
			if (_opt.button.act == true)
			{
				...
				var _button_width = ((_opt.button.width != 0) ? _opt.button.width : ((_opt.button.xDist * 2) + fn_text_width(_opt.text) + 1));
				var _button_height = ((_opt.button.height != 0) ? _opt.button.height : ((_opt.button.yDist * 2) + fn_text_height(_opt.text) + 2));
			...
			{
				var _select_x = ((_opt.select.x != 0) ? _opt.select.x : (_opt.x - ((fn_text_width(textdata(_opt.text)) / 2) * (_opt.xAlign == fa_center)) - _opt.select.xDist));
				var _select_y = ((_opt.select.y != 0) ? _opt.select.y : (_opt.y - _opt.select.yDist));
				var _select_width = ((_opt.select.width != 0) ? _opt.select.width : ((_opt.select.xDist * 2) + fn_text_width(textdata(_opt.text)) + 1));
				var _select_height = ((_opt.select.height != 0) ? _opt.select.height : ((_opt.select.yDist * 2) + fn_text_height(textdata(_opt.text)) + 2));
			...
			if (_opt.icon.spr != -1)
			{
				...
				var _icon_y = ((_opt.icon.y != 0) ? _opt.icon.y : (_opt.y + (fn_text_height(_opt.text) / 2) + 1));
			...
			{
				var _val = _opt.value;
				var _val_x = ((_val.x != 0) ? _val.x : (_opt.x + fn_text_width(_opt.text) + _opt.value.xDist));
